...
export function CommunePanel({
  ...
  return <div className="text-title">
    <div className="flex items-center gap-2 border-b border-hairline px-3 py-2">
      ...
      <span className="ml-auto font-mono text-note text-ink-muted">xã {code}</span>
    </div>
    {!feature && <p className="p-3 text-body text-ink-muted">Không có xã/phường này trong dataset đang mở.</p>}
    {props && <>
      <header className="border-b border-hairline px-3 py-3">
        <h2 className="text-heading font-semibold leading-tight">{String(props["commune_name"] ?? "—")}</h2>
        <p className="pt-0.5 text-body text-ink-muted">{formatValue(props["area_km2"] as CellValue ?? null)} km² · {formatValue(props["population"] as CellValue ?? null)} người</p>
        {current && <div className="pt-2"><div className="text-body text-ink-2">{current.label}</div><div className="text-readout leading-tight">{formatValue(props[current.column] as CellValue ?? null, current)}{props[current.column] != null && <span className="pl-1 text-body text-ink-muted">{baseUnitPhrase(current.unit)}</span>}</div></div>}
      </header>
      <section>
        <h3 className="border-b border-hairline bg-basemap px-3 py-1 text-body tracking-[0.1em] text-ink-2">BẰNG CHỨNG</h3>
        <p className="px-3 pb-1 pt-2 text-body leading-snug text-ink-muted">Tách tử số, mẫu số và quality flag; không dùng tổng thô làm kết luận công bằng.</p>
      ...
      <section className="border-t border-hairline px-3 py-2">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {FIELD_BY_ID.has("commune:ports_per_10k_pop") && <button onClick={() => setField("commune:ports_per_10k_pop")} className="cursor-pointer border border-hairline px-2 py-1 text-body hover:bg-basemap">so sánh cổng/10k dân</button>}
          {FIELD_BY_ID.has("commune:dist_station_m_pop_weighted") && <button onClick={() => setField("commune:dist_station_m_pop_weighted")} className="cursor-pointer border border-hairline px-2 py-1 text-body hover:bg-basemap">xem distance theo dân</button>}
        </div>
      </section>
      <details className="border-t border-hairline">
        <summary className="cursor-pointer px-3 py-2 text-body text-ink-2">Chi tiết dữ liệu xã/phường</summary>
